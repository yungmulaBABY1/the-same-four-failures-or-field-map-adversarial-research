...
**Author:** yungmulaBABY ([yungmulababy.substack.com](https://yungmulababy.substack.com))  
**Repository:** [github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research)  
**Full evidence bundle:** DOI [10.5281/zenodo.20201226](https://doi.org/10.5281/zenodo.20201226)  
**Five final papers:** DOI [10.5281/zenodo.20209372](https://doi.org/10.5281/zenodo.20209372)

---

## Abstract

This document describes a replicable method for identifying and documenting institutional research failures — cases where a field’s apparent consensus rests on weak proxies, circular citation networks, and a decisive test that was never run.

The method uses three diagnostic prompts, adversarial AI collaboration, and a structured evidence-archiving protocol to produce publishable methodological challenges from domain expertise alone.

It was developed through a five-paper body of work challenging the dominant academic literature on gender and elite esports performance. All prompts, worked examples, and resulting papers are publicly archived. The method is domain-agnostic. The bottleneck is domain knowledge, which practitioners already have.

---

## I. Background: The Problem the Method Addresses

Academic literatures can develop apparent consensus not through converging independent evidence, but through the accumulation of citations to claims that were never directly tested. A small founding cluster of papers establishes a premise. A review paper or meta-analysis amplifies it. Downstream papers cite the review rather than inspecting the original evidence. Limitations disappear at each citation step. The conclusion grows stronger through repetition than the original methods supported.

Adjacent literatures with directly relevant expertise remain structurally excluded from the citation network. The decisive test — the evidence that would actually resolve the question — is never specified or run.
...
2. **Proxy that measures the wrong thing** — a metric presented as evidence for a construct it cannot actually measure.
3. **Level-of-analysis error** — findings from one population or setting exported to explain outcomes in a different population or setting.
4. **Ratchet** — an intervention that expands when the metric improves, without verifying that the underlying construct has changed.
...
You do not need GitHub, Zenodo, a university login, or any technical infrastructure to run this method. All you need is access to a free AI chat interface and domain knowledge. Grok ([grok.com](https://grok.com)), Claude ([claude.ai](https://claude.ai)), and ChatGPT ([chat.openai.com](https://chat.openai.com)) all have free tiers that are sufficient for this work. The prompts in this document can be copied directly into any chat window.
...
**Stage 1: Identify the anomaly.** Domain expertise generates the initial suspicion. A practitioner, clinician, or experienced domain participant notices that a field’s published conclusions do not match observable reality. This suspicion — what the method calls “vibes” — is the valid starting point. It is not sufficient as final evidence.

**Stage 2: Run the construct validity audit.** The foundational paper — the study that everyone cites as having settled the question — is subjected to a structured 14-section audit. The audit maps what the study actually measured versus what it claimed, documents the proxy problem, identifies omitted confounds, checks ecological validity, specifies the decisive test, tracks how later papers used the finding, and identifies ignored adjacent literatures.

This audit produces a publishable claim-vs-measurement analysis.

**Stage 3: Run the citation network audit.** The citation pathway from the foundational paper to its downstream amplification hub is mapped. The audit identifies founding cluster composition, claim mutation across citation steps, adjacent-field neglect, hub-dependency behavior, critique uptake or non-uptake, and a cartel-risk score across ten dimensions. This audit produces a publishable citation network analysis.

**Stage 4: Run the field diagnostic.** Forty-four questions across all four failure modes are applied to the field as a whole. This audit identifies whether the field has a mechanism for self-correction, whether the ratchet is operating, whether the decisive test has been specified, and whether the intervention metric is improving while the underlying construct remains unmeasured.

**Stage 5: Run the adversarial loop.** After the core argument is stable, the strongest counterarguments are generated by asking an AI model to make the best case that the audit is wrong. Those counterarguments are then brought back to a different model for evaluation: “Do you find these objections compelling, or is this compliance hedging?” What survives this loop is the publishable challenge.
...
All three prompts are publicly available in the [GitHub repository](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research). They are free, reusable, and domain-agnostic.

### Prompt 1: Construct Validity Audit

**File:** [`construct_validity_claim_vs_measurement_audit_prompt.txt`](construct_validity_claim_vs_measurement_audit_prompt.txt)  
**Length:** 406 lines, 10.4 KB  
...
1. Exact claim extraction: author claim, defensible claim, inflated downstream claim.
2. Measurement extraction: what was and was not measured.
3. Claim vs. measurement table with severity ratings.
4. Level-of-analysis check.
5. Confound audit.
6. Ecological validity check.
7. Decisive-test specification.
8. Citation-use audit.
9. Ignored-literature check.
10. Source-status labels.
11. Verdict: seven options from “valid evidence” to “fatal construct-validity failure.”
12. Paper-safe wording: four versions.
13. Failure-mode classification checklist.
...
**File:** [`citation_network_cartel_adjacent_field_neglect_diagnostic_prompt.txt`](citation_network_cartel_adjacent_field_neglect_diagnostic_prompt.txt)  
**Length:** 658 lines, 19.7 KB  
...
**File:** [`field_diagnostic_44_questions_prompt.txt`](field_diagnostic_44_questions_prompt.txt)  
**Length:** approximately 400 lines  
**Purpose:** Full field-level diagnostic. Forty-four questions across all four failure modes. Identifies whether the field has the structural pattern at scale.

**Four sections:**

1. The untested premise.
2. The proxy problem.
3. The level-of-analysis error.
4. The ratchet.
...
**Summary of findings:** The study measured MMO character level advancement speed — the rate at which a character moved through level milestones per hour of login time in EverQuest II and Chevaliers’ Romance III. All max-level characters were deleted from the dataset to maintain a linear model.
...
**→ Public Substack companion:** [How a Login Timestamp Became Proof](https://yungmulababy.substack.com/p/how-a-login-timestamp-became-proof)
...
**Summary of findings:** The claim mutation was tracked across five stages:
...
**Summary of findings:** All four failure modes are present and documented.
...
**→ Read:** [Biological Sex Differences and Elite Competitive Gaming Performance](https://yungmulababy.substack.com/p/biological-sex-differences-and-elite-950)
...
**→ Read:** [Biological Sex Differences and Elite Competitive Gaming: A Response to the Cultural Literature](https://yungmulababy.substack.com/p/biological-sex-differences-and-elite)

### Meta-paper — A Jobless High School Dropout Out-Researched an Academic Field in 11 Days on an iPhone

Documents the process: which model did what, which prompts cracked the methodology open, and how the adversarial loop worked. Full receipts, timestamped, archived.

**→ Read:** [A Jobless High School Dropout Out-Researched an Academic Field in 11 Days on an iPhone](https://yungmulababy.substack.com/p/a-jobless-high-school-dropout-out)
...
**→ Read:** [The Pathologization of Excellence](https://yungmulababy.substack.com/p/the-pathologization-of-excellence)
...
**→ Read:** forthcoming on Substack.

**Full archive:** DOI [10.5281/zenodo.20209372](https://doi.org/10.5281/zenodo.20209372) five papers  
**Full evidence bundle:** DOI [10.5281/zenodo.20201226](https://doi.org/10.5281/zenodo.20201226)
...
**Grok** — initial diagnostic mapping, direct engagement with contested empirical claims, willingness to name failure modes without softening. Use for running the diagnostic prompts, identifying proxy problems, and characterizing the citation network. Grok will tell you directly when the answer is no.

**Claude** — long-form structure, synthesis across sources, cross-document consistency, argument architecture. Use for organizing the argument, checking internal consistency, producing readable documents, and systematic cross-referencing.

**ChatGPT** — adversarial stress-testing after the core argument is stable. Use for generating the strongest objections to your challenge, identifying the weakest points, and finding where evidence is thinnest. Do not use for initial diagnostic work.
...
> Has the field’s confidence exceeded what its evidence can support, and has it failed to run the test that would resolve the question?
...
2. **Fill in the construct validity audit input template** — abstract, methods, how later papers cite it, your domain-expert concern.
3. **Paste into Grok** — for the most direct assessment.
4. **Paste into Claude** — for structured synthesis and document production.
5. **Identify the amplification hub** — the review paper that converted the weak finding into review authority.
6. **Run the citation network audit** — map the claim mutation, the structural holes, and the cartel-risk score.
7. **Run the adversarial loop** — ChatGPT generates counterarguments; Grok evaluates them.
8. **Archive everything** — public evidence bundle, source-status labels, correction log.
9. **Specify the decisive test** — one sentence stating what evidence would settle the question.
10. **Publish** — the method is the receipts.
...
**Repository:** [github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research)  
**Papers archive:** DOI [10.5281/zenodo.20209372](https://doi.org/10.5281/zenodo.20209372)  
**Full evidence bundle:** DOI [10.5281/zenodo.20201226](https://doi.org/10.5281/zenodo.20201226)
