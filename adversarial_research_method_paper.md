...
> domain suspicion → construct audit → citation audit → field diagnostic → adversarial stress test → public receipts
...
**Stage 2: Run the construct validity audit and data generation check.**
The foundational paper is subjected to a structured 14-section audit. Before the construct validity audit runs, the data generation audit establishes whether the underlying data is sound: how it was collected, what selection effects shaped it, whether missingness is systematic, and what incentive structure governed collection. For AI-assisted analysis, the adversarial check is run at this stage — forcing the model to surface its own proxy failures before the output is treated as a finding. A claim built on flawed data AND a bad proxy AND a non-replicating finding is triply invalid. The data generation audit catches the first failure before the other two are even assessed. This audit produces a publishable claim-vs-measurement analysis.
...
### Prompt 4: Oppositional Counter-Audit
...
### Prompt 5: Internal Validity and Replication Audit
