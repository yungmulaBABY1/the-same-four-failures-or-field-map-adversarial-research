...
- **Off-Label Use Audit Prompt** *(addendum)* — audits the evidence base for drugs, interventions, or protocols used outside their approved indication, population, or duration. The organizing concept is the evidence transfer problem: the study was conducted in population A under conditions B; the use is being justified for population C under conditions D. Covers evidence quality, normalization pathway, financial and institutional incentives, harm signals, and reversibility. Verdict scale from evidence-based through potentially harmful. Candidate applications include puberty blockers in minors, GLP-1 agonists for non-diabetic weight loss, and SSRIs in adolescents. Run standalone or in combination with the Construct Validity and Internal Validity prompts for a complete three-layer assessment.
...
**Addendum prompts:** The toolkit also includes domain-specific audit prompts that extend the core methodology into specialized areas. These live in the repo as standalone files and are designed to be run alongside the main prompts rather than replacing them.
