...
The toolkit has two entry points depending on what you are starting with.

**Domain expert entry** — you already have a specific study or claim that doesn’t sit right. You have the practitioner suspicion. Start with the construct validity audit on that study and work outward. The nine-step workflow below is built for this path.

**Discovery entry** — you have a general concern about a field, condition, or drug category but no specific study in hand. Start with the 44-question field diagnostic as a triage tool: *“Run the 44-question field diagnostic on [field/condition/drug category] — identify the most common claims or interventions, which have the weakest evidence bases, and which decisive tests haven’t been run.”* The diagnostic surfaces the highest-risk audit targets without requiring prior domain knowledge. Then run the specific audits on what it finds. This is also the natural path for non-experts who know something smells wrong but don’t yet know where to look.

**Domain expert path — nine steps:**

1. Read the [method paper](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/blob/main/adversarial_research_method_paper.md) — understand the method before running the prompts.
1. Run the [construct validity audit](https://raw.githubusercontent.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/main/construct_validity_claim_vs_measurement_audit_prompt.txt) — does the study measure what it claims?
1. Run the [internal validity audit](https://raw.githubusercontent.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/main/internal_validity_replication_audit_prompt.txt) — is the finding even real?
1. Run the [citation network audit](https://raw.githubusercontent.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/main/citation_network_cartel_adjacent_field_neglect_diagnostic_prompt.txt) — did confidence accumulate through evidence or citations?
1. Run the [field diagnostic](https://raw.githubusercontent.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/main/field_diagnostic_44_questions_prompt.txt) — is this one bad paper or is the whole field like this?
1. Add your domain knowledge — what do practitioners know that the literature ignores?
1. Run the [oppositional counter-audit](https://raw.githubusercontent.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/main/oppositional_counter_audit.txt) — what does the strongest opposing case look like?
1. Run [voice restoration](https://raw.githubusercontent.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/main/voice_restoration_anticipatory_compliance_audit_prompt.txt) — did the AI soften the critique until it apologized for existing?
1. Publish with receipts — archive everything publicly, make the challenge inspectable and replicable.
...
- **Construct Validity Audit Prompt** — forces the model to check whether the claimed construct actually matches what was measured. The single most powerful tool in the kit. Now includes an **Off-Label Use Flag** (Section 13-A) that routes to the standalone off-label audit when triggered.
- **Internal Validity & Replication Audit Prompt** — audits whether the foundational finding is real and stable before asking what it proves. Covers pre-registration, p-hacking, HARKing, selective reporting, effect size plausibility, replication status, and data availability. Now includes a data generation audit (selection effects, missingness, incentive structure) and an AI-assisted analysis check that forces the model to surface proxy failures before output is treated as a finding.
- **Citation Network Cartel Detector** — identifies closed citation clusters, claim mutation, and adjacent-field neglect
- **44-Question Field Diagnostic** — the complete field-level audit checklist, scalable to institutional-level audits
...
- **Synthesis & Integration Prompt** (`synthesis_integration_prompt.txt`) — runs after two or more individual audits are complete. Forces integration of findings across prompts into one prioritized, non-hedged verdict. Seven structured steps: identify the most important findings, explain how they compound, assess source-status tier of load-bearing claims, evaluate strength of the core claim, produce an overall verdict (four constrained options), write a concise publishable challenge statement, and specify the decisive test that would change the verdict. Explicitly designed to prevent the hedging and false balance that models default to when synthesizing across multiple analytical outputs.
- **Audit Template** (`AUDIT_TEMPLATE.md`) — copy-paste template for producing your own structured audit. Blank fields for claim, prompts used, source-status assessment, findings, and verdict. Includes three worked examples: the IAT 27-year proxy problem (construct validity + citation network + oppositional counter-audit), the Donnon reversal in the MCAT debate (citation-use audit), and the Shen et al. esports gender literature (full five-prompt audit). Shows different prompt combinations and failure modes.
- **Off-Label Use Audit Prompt** *(domain-specific extension — run when auditing medical claims, drug prescribing, or clinical interventions)* — audits the evidence base for drugs, interventions, or protocols used outside their approved indication, population, or duration. The organizing concept is the evidence transfer problem: the study was conducted in population A under conditions B, used to justify population C under conditions D. Covers evidence quality, normalization pathway, financial incentives, harm signals, and reversibility. Verdict scale from evidence-based through potentially harmful. Composable with the other prompts in a single session — frame the input as “audit [drug] used off-label for [population]” and the model applies each framework in sequence.
- **Source-Status Hierarchy** — a nine-tier classification system for labeling evidence strength during an audit. Forces explicit identification of what tier every load-bearing claim rests on. Tier 1 (primary data) through Tier 9 (author speculation). Prevents overclaiming, makes audits reproducible, and catches citation laundering — where a low-tier claim accumulates unearned authority by passing through progressively higher-tier citations without the original weakness being preserved. Full framework in the method paper.
...
The natural entry point is a specific study or claim that doesn’t sit right — not an abstract field. Start with the study, audit it, then zoom out to ask whether the problem is structural.
...
**2. Run the Construct Validity Audit** — use [construct_validity_claim_vs_measurement_audit_prompt.txt](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/blob/main/construct_validity_claim_vs_measurement_audit_prompt.txt) on the specific study or claim that triggered your suspicion. What did it actually measure? What did it claim? What is the gap? This is the most accessible audit and the natural first step — it doesn’t require hunting replication literature, just a claim and a measurement.

**3. Run the Internal Validity Audit** — use [internal_validity_replication_audit_prompt.txt](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/blob/main/internal_validity_replication_audit_prompt.txt). Now that you know what the study claims to measure, ask whether the finding is even real. Starts with a data generation audit: how was the data collected, what selection effects shaped it, is missingness systematic, what incentive structure governed collection? Then: was the finding pre-registered? Has it replicated? Is the effect size stable or inflated by p-hacking? Only worth the technical legwork once you know the claim is worth stress-testing.

**4. Run the Citation Network Audit** — use [citation_network_cartel_adjacent_field_neglect_diagnostic_prompt.txt](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/blob/main/citation_network_cartel_adjacent_field_neglect_diagnostic_prompt.txt). Did the claim become stronger because evidence accumulated, or because citations accumulated? Map the founding cluster, claim mutation, and adjacent-field neglect.

**5. Run the Field Diagnostic** — use [field_diagnostic_44_questions_prompt.txt](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/blob/main/field_diagnostic_44_questions_prompt.txt). Now zoom out: is this one bad paper or is the whole field like this? The field diagnostic answers whether the problem is isolated or structural — it’s a follow-up question, not a starting question.

**6. Bring your domain knowledge** — add what the prompts couldn’t find: practitioner observations, unindexed community knowledge, adjacent fields the literature ignored. Test your additions conversationally: “Is this actually supported or am I motivated reasoning?”

**7. Run the Oppositional Counter-Audit** — use [oppositional_counter_audit.txt](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/blob/main/oppositional_counter_audit.txt). Find the strongest evidence for the other side. What did the audit miss? What evidence would change the conclusion? A good audit becomes sharper after this. A bad audit should be abandoned.

**7.5. Run the Synthesis & Integration Prompt** — use [synthesis_integration_prompt.txt](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/blob/main/synthesis_integration_prompt.txt). After running multiple individual audits, integrate the findings into one prioritized, non-hedged verdict. How do the failure modes compound? What tier is the load-bearing evidence on? What is the single most important finding? What is the concise publishable challenge statement? This is where models default to hedging without strong constraints — run this before drafting the final output.

**8. Run Voice Restoration** — use [voice_restoration_anticipatory_compliance_audit_prompt.txt](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/blob/main/voice_restoration_anticipatory_compliance_audit_prompt.txt). Did the AI soften the critique until it apologized for existing?

## **9. Publish with receipts** — archive everything publicly (Zenodo or GitHub). Include the prompts used, model responses, and evidence bundle. Make the challenge inspectable and replicable.
...
The toolkit was developed through academic literature audits but the prompts scale to any system that produces claims and has an institutional structure around those claims. The core question — does the metric measure the same thing as the claim — applies wherever there is a stated principle and a measurable behavior.

**Institutional audits:** The 44-question field diagnostic works against an institution’s output corpus directly, not just an academic field. Frame the input as: “Run this diagnostic against [Institution X] — treating their published research output, policy positions, and public communications as the field to audit.” Candidates include:

- The APA — clinical guidelines, the masculinity guidelines specifically, and their response to replication failures in social psychology
- The AANP — NP scope-of-practice advocacy versus the supervised-setting evidence base
- The AAMC — medical admissions claims versus the downstream performance literature
- Project Implicit as an institution — not just the IAT as a study

Institutional-level audit questions: Does the institution cite its own researchers disproportionately? Does it engage with falsifying evidence or dismiss it? Does it have a mechanism for updating when decisive tests fail? Does its public communication match what its own research shows?

**Political and policy audits:** The construct validity prompt applies directly to party platforms, candidate records, regulatory agency outputs, and think tank publications. Stated principle versus actual behavior is the core question regardless of what you’re auditing. A party platform claiming fiscal responsibility versus actual spending record. A candidate claiming to support X versus voting record on X. A regulatory agency claiming evidence-based policy versus which evidence it actually cites and which it ignores.

**Media audits:** The citation network prompt works on news organizations. Which sources get cited, which experts get quoted repeatedly, which studies get amplified versus buried — that is a citation network operating in journalism rather than academia. The diagnostic questions are identical. Coverage pattern analysis, source concentration, and asymmetric amplification of findings are all auditable with the same framework.

**AI-assisted data analysis:** The internal validity prompt now includes an explicit AI analysis check. Before treating any AI-generated analysis as a finding, run the adversarial check: “What is the strongest methodological critique of this analysis? What does this measure that I might be mistaking for what I want it to measure? What would falsify this finding?” The data generation audit section establishes whether the underlying data is sound before any analysis questions are asked.

**Domain-specific extensions:** The toolkit includes specialized audit prompts for particular domains. These are optional — run them when the domain applies, skip them when it doesn’t. They extend the core methodology rather than replacing it, and are designed to be run alongside the main prompts.

- **Off-Label Use Audit** ([off_label_use_audit_prompt.txt](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/blob/main/off_label_use_audit_prompt.txt)) — for drugs, interventions, or protocols used outside approved indications. The Construct Validity prompt flags off-label issues automatically in Section 13-A and routes here when two or more triggers fire. All four main prompts are composable in a single session — frame the input as “audit [drug] used off-label for [population]” and the model applies each framework in sequence without needing separate sessions. Example: “Run the 44-question field diagnostic treating GLP-1 use in non-diabetic adolescents as the field, then the construct validity audit on the primary supporting studies, then the off-label evidence transfer assessment.” The 44-question diagnostic, construct validity, internal validity, citation network, and off-label prompts all understand domain context and apply correctly from natural language framing.

**The real scope of the toolkit:** Auditing any institution, field, or actor that makes empirical claims and has a measurement framework around those claims. Academic literature is where it was developed. It is not where it stops.
...
|[oppositional_counter_audit.txt](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/blob/main/oppositional_counter_audit.txt)                                                                            |**Run this after the other audits.** Forces the question: what did the audit miss? What does the other side have? What evidence would change the conclusion? Tests whether the audit survives contact with the strongest opposing evidence. A good audit becomes sharper after this. A weak audit becomes narrower. A bad audit should be abandoned.                                                                                                                                                                                                                                                                                                                                                                                                        |
|[internal_validity_replication_audit_prompt.txt](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/blob/main/internal_validity_replication_audit_prompt.txt)                                            |**Run before or alongside the construct validity audit.** Audits whether the foundational finding itself is real and stable — now includes a **data generation audit** (how was the data collected, what selection effects shaped it, is missingness systematic, what incentive structure governed collection) plus an **AI-assisted analysis check** that forces the model to surface proxy failures before output is treated as a finding. Also covers p-hacking, HARKing, selective reporting, replication status, effect size trajectory, and data availability. A finding that fails here is doubly weak before the construct validity audit even runs. A finding that fails on data generation, replication, AND construct validity is triply invalid.|
...
**1. Identify the specific claim** — State in one sentence what the study, institution, or actor treats as established fact. Start with what triggered your suspicion.

**2. Audit the proxy** — Run the construct validity audit. Extract the exact metric measured and the exact claim made. Does this metric validly support the claim? This is the most accessible step and requires no technical background — just the claim and the measurement.

**3. Check whether the finding is real** — Run the internal validity audit. Was the study pre-registered? Has it replicated? Is the effect size stable? Check the data generation: selection effects, missingness, incentive structure. Only worth this legwork once you know the claim is worth stress-testing.

**4. Map the citation network** — Identify the top 10–15 most-cited papers and the founding cluster. Calculate internal citation density. Did the claim become stronger through evidence or through citation recycling?

**5. Run the field diagnostic** — Now zoom out. Use the 44-question diagnostic to ask: is this one bad paper or is the whole field like this? The diagnostic answers whether the failure is isolated or structural.

**6. Apply the prompting workarounds** — Construct-Validity Reframe, Direct Challenge, Technical/Narrow Framing.

**7. Stress-test with the Oppositional Counter-Audit** — Find the strongest evidence for the other side. What did the audit miss? What evidence would change the conclusion?
...
In fields where foundational premises have been repeatedly tested and survived, citation networks function on warranted trust. Running a construct validity audit on Newton’s laws would produce a clean bill of health. That is the correct result.

But disciplinary label is not the right boundary. String theory is mathematically sophisticated and built on decades of peer-reviewed literature — and has not produced a single falsifiable prediction that has been tested at the scale the theory requires. The decisive test has never been run. The citation network is enormous. The prestige is enormous. Same failure modes as contested soft science fields. Different discipline label.

The toolkit applies wherever: the foundational premise was assumed rather than directly tested; the citation network treats that assumption as established fact; the proxy and the construct are different things; domain experts can see the gap but lack the formal pathway to document it; adjacent fields with better measurement instruments were structurally excluded; or the decisive test has not been run — or has been run and the field did not update.
