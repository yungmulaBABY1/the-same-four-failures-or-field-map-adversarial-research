...
**2. Run the Internal Validity Audit** — use [internal_validity_replication_audit_prompt.txt](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/blob/main/internal_validity_replication_audit_prompt.txt). Now starts with a data generation audit: how was the data collected, what selection effects shaped it, is missingness systematic, what incentive structure governed collection? Then: was the finding pre-registered? Has it replicated? Is the effect size stable or inflated by p-hacking? For AI-assisted analysis: was the adversarial check run before treating output as a finding? A finding that fails here is doubly weak before the construct validity audit even runs.

**3. Run the Construct Validity Audit** — use [construct_validity_claim_vs_measurement_audit_prompt.txt](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/blob/main/construct_validity_claim_vs_measurement_audit_prompt.txt) on your field’s foundational study. What did it actually measure? What did it claim?
...
**5. Run the Field Diagnostic** — use [field_diagnostic_44_questions_prompt.txt](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/blob/main/field_diagnostic_44_questions_prompt.txt). Is this an isolated failure or a structural pattern across the field?
