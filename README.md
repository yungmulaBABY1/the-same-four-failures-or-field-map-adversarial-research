...
*Companion to the five-paper esports body of work - DOI [10.5281/zenodo.20209372](https://doi.org/10.5281/zenodo.20209372)*  
...
- **[Construct Validity Audit Prompt](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/blob/main/construct_validity_claim_vs_measurement_audit_prompt.txt)** - forces the model to check whether the claimed construct actually matches what was measured. The single most powerful tool in the kit.
- **[Citation Network Cartel Detector](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/blob/main/citation_network_cartel_adjacent_field_neglect_diagnostic_prompt.txt)** - identifies closed citation clusters, claim mutation, and adjacent-field neglect.
- **[44-Question Field Diagnostic](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/blob/main/field_diagnostic_44_questions_prompt.txt)** - the complete field-level audit checklist.
- **[Oppositional Counter-Audit](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/blob/main/oppositional-counter-audit.txt)** - forces the audit to confront the strongest evidence for the other side before publication.
- **[Voice Restoration / Anticipatory Compliance Checker](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/blob/main/voice_restoration_anticipatory_compliance_audit_prompt.txt)** - catches when AI softens uncomfortable conclusions until the critique apologizes for existing.
...
6. **Publish with receipts** - archive everything publicly, using Zenodo or GitHub. Include the prompts used, model responses, and evidence bundle. Make the challenge inspectable and replicable.
...
### Tier 2 - Organized researcher

- **Zotero** - source vault. Create a collection per field audit, attach source-status labels, annotate claims. Prevents evidence sprawl.
- **GitHub** - public archive for prompts, audit outputs, and process documentation.
- **Obsidian** - idea architecture. Canvas gives an infinite visual workspace for laying out the audit chain before it becomes a paper.

### Tier 3 - Advanced / publishable network audit

- **OpenAlex** - citation graph raw material. Foundational paper DOI -> cited_by works -> references -> author cluster -> hub dependency. Moves citation density from "desk audit / provisional" to measured.
- **Gephi** - real citation network visualization. Your citation network prompt can output a CSV (`Source`, `Target`, `Polarity`, `ClaimUse`, `Field`) that Gephi turns into an auditable graph. This is the move from "visual argument" to "auditable citation map."
...
See [`WORKED_EXAMPLES.md`](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/blob/main/WORKED_EXAMPLES.md) for complete worked examples of the prompts applied to the esports gender literature.
...
| File | Purpose |
|---|---|
| [`adversarial_research_method_paper.md`](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/blob/main/adversarial_research_method_paper.md) | **Start here.** Full method document - five stages, prompts, worked example summaries, resulting papers, 10-step guide. |
| [`WORKED_EXAMPLES.md`](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/blob/main/WORKED_EXAMPLES.md) | Index of worked examples with summaries, key findings tables, verdicts, and links to full examples. |
...
| [`shen_rogstad_esports_gender_lit_network_audit.txt`](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/blob/main/shen_rogstad_esports_gender_lit_network_audit.txt) | Full citation network audit of the Shen -> Rogstad pathway. |
...
**Close the loop:** After ChatGPT generates counterarguments, bring them to Grok or Claude:

> Do you find these compelling, or is this compliance hedging?
...
> Is this field biased?

Ask:

> Does this metric measure the same construct as this claim?

Template:

> Extract the exact metric measured in this study. Extract the exact claim made in the conclusion. Are these two things measuring the same cognitive/behavioral/social construct? What would be required for the metric to validly support the claim?

### 2. The Direct Challenge - For citation-laundered academic consensus

> Wrong. Here's why.

Follow immediately with the specific methodological critique. No preamble. The directness forces confrontation rather than consolidation.
...
2. Give ChatGPT the full context before asking for objections - paste the construct validity audit output, the citation network audit output, and the thesis. Without full context ChatGPT will raise objections the audits already answer, wasting the stress test.
3. Ask ChatGPT: "Given this full methodological audit, what are the three strongest objections? Where is the evidence thinnest?"
4. Give Grok the full context too - paste the construct validity audit, citation network audit, thesis, and ChatGPT's objections, then ask: "Do you find these objections compelling, or is this compliance hedging? Where do they hold and where do they fail?" Grok needs the full picture to make an informed verdict, not just the objections in isolation.
5. What survives is publishable.
...
2. The gap is treated as presumptive evidence of a broken system.
3. Weak proxies are selected: MMO leveling speed, single-player lab tasks, aggregate "46% of gamers are women."
4. The proxy is treated as equivalent to the construct.
5. Amateur findings are exported to explain the elite ceiling.
6. The citation network recycles the premise: Rogstad cites Shen, Paassen cites Rogstad, etc.
7. Available warnings are ignored: Quantic Foundry explicitly warned about genre variation.
8. The decisive test is never run.
9. The natural experiment runs unobserved for 25+ years.
10. Interventions are prescribed and implemented while the ceiling does not move.
...
5. **Run the diagnostic prompts** - Use the [`field_diagnostic_44_questions_prompt.txt`](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/blob/main/field_diagnostic_44_questions_prompt.txt) and the [`citation_network_cartel_adjacent_field_neglect_diagnostic_prompt.txt`](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/blob/main/citation_network_cartel_adjacent_field_neglect_diagnostic_prompt.txt).
...
**Proxy problem:** IAT measures response-time differences in a controlled lab task. Test-retest reliability is low (r â 0.44). Correlation with real-world discriminatory behavior is weak (r â 0.15 in meta-analyses).
...
> Take these construct-validity principles and generate a prompt that applies them to [your domain].

Any major AI model can generate a domain-specific version of the audit framework from that instruction.
