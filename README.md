...
**2. Run the Internal Validity Audit** — use [internal_validity_replication_audit_prompt.txt](https://github.com/yungmulaBABY1/the-same-four-failures-or-field-map-adversarial-research/blob/main/internal_validity_replication_audit_prompt.txt). Was the finding pre-registered? Has it replicated? Is the effect size stable or inflated by p-hacking? A finding that fails here is doubly weak before the construct validity audit even runs.
